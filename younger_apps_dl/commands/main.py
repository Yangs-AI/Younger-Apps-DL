...
def option(ctx: click.Context, task_kind: str, task_name: str, task_step: str, toml_path: pathlib.Path, logging_filepath: pathlib.Path):
    ...
    :type task_name: str
    :param task_step: Indicates the step of task. If provided, only options for this step will be included.
    :type task_step: str
    ...
    # Generate configuration template based on task step
    stage_fields = Task.STAGE_REQUIRED_OPTION.get(task_step, list()) if task_step != 'All' else None
    helping_lines = generate_helping_for_pydantic_model(Task.OPTIONS, include_fields=stage_fields)
    helping = '\n'.join(helping_lines)
    helping = (
        f'# Configuration template for "<{task_kind}> -> <{task_name}>" task (Step: {task_step}).\n'
        f'# Generated by Younger Apps DL.\n\n'
        f'# This configuration template includes the options required for "{task_step}" step.\n'
        f'# One can split the configuration into multiple files and include them using the "include" directive in TOML.\n'
        ...
        f'# Run:\n'
        f'#     younger apps dl launch --task-kind {task_kind} --task-name {task_name} --toml-path {toml_path} {"" if task_step is None else "--task-step " + task_step}\n\n'
    ) + helping
    toml_path.write_text(helping, encoding='utf-8')
    click.echo(f'Configuration template for "<{task_kind}> -> <{task_name}>" task (Step: {task_step}) has been written to the file - "{toml_path.absolute()}".')
