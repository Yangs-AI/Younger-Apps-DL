...
# Last Modified time: 2026-01-12 17:07:22
...
class VGAENodeClassificationEncoder(nn.Module):
    """
    Encoder for Variational Graph Autoencoder - wrappable by torch_geometric.nn.VGAE.

    Returns (mu, logvar) for reparameterization by VGAE wrapper.
    """

    def __init__(self, node_emb_size, node_emb_dim, hidden_dim):
        super(VGAENodeClassificationEncoder, self).__init__()
        self.node_embedding_layer = Embedding(node_emb_size, node_emb_dim)
        self.layer_1 = GCNConv(node_emb_dim, 2 * hidden_dim)
        self.layer_mu = GCNConv(2 * hidden_dim, hidden_dim)
        self.layer_logvar = GCNConv(2 * hidden_dim, hidden_dim)
        self.initialize_parameters()

    def forward(self, x, edge_index):
        x = self.node_embedding_layer(x).squeeze(1)
        x = F.relu(self.layer_1(x, edge_index))
        mu = self.layer_mu(x, edge_index)
        logvar = self.layer_logvar(x, edge_index)
        return mu, logvar
...
class VGAENodeClassificationClassifier(nn.Module):
    """
    Linear Classifier for VGAE Node Classification.
    """

    def __init__(self, hidden_dim, node_emb_size):
        super(VGAENodeClassificationClassifier, self).__init__()
    ...
    def forward(self, x, x_position = None, embedding = False):
        ...
        Args:
            ...
            x_position: Positions of nodes to classify.
...
@register_model("vgae_node_classification")
class VGAENodeClassification(nn.Module):
    """
    Unified VGAE model with encoder and classifier.
    ...
    def __init__(self, node_emb_size: int, node_emb_dim: int, hidden_dim: int, stage: Literal['encode', 'classify', 'embedding'] = 'classify'):
        ...
        self.stage = stage
        if stage == 'encode':
            self.encoder = VGAE(VGAENodeClassificationEncoder(node_emb_size, node_emb_dim, hidden_dim))
        if stage == 'classify':
            self.encoder = VGAE(VGAENodeClassificationEncoder(node_emb_size, node_emb_dim, hidden_dim))
            gradient_computation_switch(self.encoder, False)
            self.encoder.eval()
            self.classifier = VGAENodeClassificationClassifier(hidden_dim, node_emb_size)
        if stage == 'embedding':
            self.encoder = VGAE(VGAENodeClassificationEncoder(node_emb_size, node_emb_dim, hidden_dim))
            self.classifier = VGAENodeClassificationClassifier(hidden_dim, node_emb_size)
